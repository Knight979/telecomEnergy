 ...
 * @LastEditors: Knight
 * @LastEditTime: 2024-05-15 09:51:25
 */
...
const SetLanguageScreen = () => {
  ...
  const handleLanguageChange = (language: LanguageType): AppThunk => dispatch => {
  ...
  return (
    <View style={styles.container}>
      ...
      <LangRow
        title={'English'}
        selected={lang === 'en'} 
        onPress={() => dispatch(handleLanguageChange('en'))}
      />
      <LangRow
        title={'简体中文'}
        selected={lang === 'zh_cn'}
        onPress={() => dispatch(handleLanguageChange('zh_cn'))}
      />
      <LangRow
        title={'繁體中文（香港)'}
        selected={lang === 'zh_hk'}
        onPress={() => dispatch(handleLanguageChange('zh_hk'))}
      />
